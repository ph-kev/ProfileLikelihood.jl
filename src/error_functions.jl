...
where η is the noise level, ``g(t_i,θ)`` is the ``i``th point of the predicted solution, 
and ``y_i`` is the ``i``th data point. 
"""
function relative_error(data::AbstractVector{<:Real}, sol::AbstractVector{<:Real}, 
                        noise_level::Real) 
...
where ``g(t_i,θ)`` is the ``i``th point of the predicted solution 
and ``y_i`` is the ``i``th data point.
"""
...
"""
    const_variance_error(data::AbstractVector{<:Real}, sol::AbstractVector{<:Real}, 
                         sigma::Real)

The objective function for when the data follow a normal distribution with mean 0 and 
...
and ``y_i`` is the ``i``th data point.
"""
function const_variance_error(data::AbstractVector{<:Real}, sol::AbstractVector{<:Real}, 
                              sigma::Real)
    return (1 / sigma^2) * sum((sol - data) .^ 2)
end

"""
    likelihood_const(obj::String; 
                     noise_level::Real=0.0, 
                     times::AbstractVector{<:Real}=Vector{Real}(), 
                     data::AbstractVector{<:Real}=Vector{Real}(), 
                     sigma::Real = -1.0) 

This gives the constant in the likelihood function when the error is assumed to follow 
relative error or constant variance error or when the data is assumed to follow a Poisson 
distribution.
