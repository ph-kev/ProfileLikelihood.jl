...
loss, fitted_params = estimate_params(p1, [noisy_data], Int64[], prob, Tsit5(), times, [obj], BBO_generating_set_search(), [eps(Float64), eps(Float64), eps(Float64)], [2.0, 2.0, 2.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts, opti_solver_opts = opti_solver_opts, print_status = true)
...
# beta_h
theta1, sol1 = find_profile_likelihood(3.33e-6, 120, 1, fitted_params, [noisy_data], Int64[], threshold_simu + 3, loss, prob, Tsit5(), times, [obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 10.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts, pl_const = pl_const, print_status = true)
...
# beta_v 
theta2, sol2 = find_profile_likelihood(4.8e-4, 150, 2, fitted_params, [noisy_data], Int64[], threshold_simu + 3, loss, prob, Tsit5(), times, [obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 100.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts, pl_const = pl_const, print_status = false)
...
# gamma
theta3, sol3 = find_profile_likelihood(1.8e-1, 180, 3, fitted_params, [noisy_data], Int64[], threshold_simu + 3, loss, prob, Tsit5(), times, [obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 1000.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts, pl_const = pl_const)
deleteat!(theta3, 1)
deleteat!(sol3, 1)
...
jldsave("sisIncidencePoissonMisfitRelativeError0_30.jld2"; loss, fitted_params, threshold_simu, threshold_poin, pl_const, theta1, sol1, theta2, sol2, theta3, sol3, ci_simu_beta_h, ci_simu_beta_v, ci_simu_gamma, ci_poin_beta_h, ci_poin_beta_v, ci_poin_gamma)