...
loss, fitted_params = estimate_params(p1, [noisy_data], Int64[], prob, Tsit5(), times, [obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [2.0, 2.0, 2.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts)
...
# beta_h
theta1, sol1 = find_profile_likelihood_multistart(1e-5, 500, 1, fitted_params, [noisy_data], Int64[], threshold_simu + 3, loss, prob, Tsit5(), times, [obj], MultistartOptimization.TikTak(300), NLopt.LN_NELDERMEAD(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 10.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts, pl_const = pl_const, print_status = true)
...
# beta_v 
theta2, sol2 = find_profile_likelihood(2e-4, 200, 2, fitted_params, [noisy_data], Int64[], threshold_simu + 0.5, loss, prob, Tsit5(), times, [obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 100.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts, pl_const = pl_const, print_status = false)
deleteat!(theta2, 1)
deleteat!(sol2, 1)
...
# gamma
theta3, sol3 = find_profile_likelihood(0.1, 1000, 3, fitted_params, [noisy_data], Int64[], threshold_simu + 3, loss, prob, Tsit5(), times, [obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 1000.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts, pl_const = pl_const)
...
jldsave("sisIncidencePoisson0_15.jld2"; loss, fitted_params, threshold_simu, threshold_poin, pl_const, theta1, sol1, theta2, sol2, theta3, sol3, ci_simu_beta_h, ci_simu_beta_v, ci_simu_gamma, ci_poin_beta_h, ci_poin_beta_v, ci_poin_gamma)