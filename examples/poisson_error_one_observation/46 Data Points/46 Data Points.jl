...
loss, fitted_params = estimate_params_multistart(p1, [noisyData], [], prob, Tsit5(), times, [obj], MultistartOptimization.TikTak(3000), NLopt.LN_NELDERMEAD(), [eps(Float64), eps(Float64), eps(Float64)], [2.0, 2.0, 2.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts)
...
# beta_h
theta1, sol1 = find_profile_likelihood(1e-6, 200, 1, fitted_params, [noisyData], [], threshold_simu + 3, loss, prob, Tsit5(), times, [obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 10.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts, pl_const = pl_const, print_status = true)
...
# beta_v 
theta2, sol2 = find_profile_likelihood(2.27e-5, 150, 2, fitted_params, [noisyData], [], threshold_simu + 0.5, loss, prob, Tsit5(), times, [obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 100.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts, pl_const = pl_const, print_status = false)
deleteat!(theta2, 1)
deleteat!(sol2, 1)
...
# gamma
theta3, sol3 = find_profile_likelihood(8.333e-3, 150, 3, fitted_params, [noisyData], [], threshold_simu + 3, loss, prob, Tsit5(), times, [obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 1000.0]; incidence_obs = [5], solver_diff_opts=solver_diff_opts, pl_const = pl_const)
