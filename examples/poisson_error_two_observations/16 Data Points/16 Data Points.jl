...
# beta_h
theta1, sol1 = find_profile_likelihood(9.999e-7, 600, 1, fitted_params, [noisy_data_host, noisy_data_vector], [], threshold_simu + 3, loss, prob, Tsit5(), times, [obj, obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 4.0]; incidence_obs = [5, 6], solver_diff_opts=solver_diff_opts, pl_const = pl_const, print_status = true)
...
# beta_v 
theta2, sol2 = find_profile_likelihood(2e-5, 55, 2, fitted_params, [noisy_data_host, noisy_data_vector], [], threshold_simu + 3, loss, prob, Tsit5(), times, [obj, obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 4.0]; incidence_obs = [5, 6], solver_diff_opts=solver_diff_opts, pl_const = pl_const, print_status = false)
...
# gamma
theta3, sol3 = find_profile_likelihood(7.2e-3, 70, 3, fitted_params, [noisy_data_host, noisy_data_vector], [], threshold_simu + 3, loss, prob, Tsit5(), times, [obj, obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 4.0]; incidence_obs = [5, 6], solver_diff_opts=solver_diff_opts, pl_const = pl_const)
