...
# beta_h
theta1, sol1 = find_profile_likelihood(8.333e-7, 60, 1, fitted_params, [noisy_data_host, noisy_data_vector], Int64[], threshold_simu + 3, loss, prob, Tsit5(), times, [obj, obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 10.0]; incidence_obs = [5, 6], solver_diff_opts=solver_diff_opts, pl_const = pl_const, print_status = true)
...
# beta_v 
theta2, sol2 = find_profile_likelihood(5e-6, 55, 2, fitted_params, [noisy_data_host, noisy_data_vector], Int64[], threshold_simu + 3, loss, prob, Tsit5(), times, [obj, obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 100.0]; incidence_obs = [5, 6], solver_diff_opts=solver_diff_opts, pl_const = pl_const, print_status = false)
deleteat!(theta2, 1)
deleteat!(sol2, 1)
...
# gamma
theta3, sol3 = find_profile_likelihood(1e-3, 55, 3, fitted_params, [noisy_data_host, noisy_data_vector], Int64[], threshold_simu + 3, loss, prob, Tsit5(), times, [obj, obj], NOMADOpt(), [eps(Float64), eps(Float64), eps(Float64)], [4.0, 4.0, 1000.0]; incidence_obs = [5, 6], solver_diff_opts=solver_diff_opts, pl_const = pl_const)
