...
# This tutorial highlights how `ProfileLikelihood.jl` can be used to generate data, estimate parameters, and find the profile likelihood. Furthermore, profile likelihood can be used for identifiability analysis and confidence intervals estimation. This is not meant to be a comprehensive tutorial using `ProfileLikelihood.jl", but is the bare minimum to get started with profile likelihood. It is recommend to read the documentation alongside the tutorial.  
